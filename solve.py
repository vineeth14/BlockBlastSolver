                ...
                while len(q) > 0:
                    for i in range(len(q)):
                        ...
                        for dr,dc in ADJACENT_DIRECTIONS:
                            nr, nc = dr+block.row, dc+block.col
                            if nr in range(BOARD_SIZE) and nc in range(BOARD_SIZE) and board[nr][nc] == 0:
                                board[nr][nc] = 1
                                q.append(Block(nr,nc))
                                count += 1
                holes.append(count)
...
def is_permutation_seen(order, already_seen, shapes):
    ...
    for permut in already_seen:
        matching_shapes = 0
        for i in range(3):
            current_shape = [(block.row, block.col) for block in shapes[order[i]].segment]
            permut_shape = [(block.row, block.col) for block in shapes[permut[i]].segment]
        ...
        for c in range(BOARD_SIZE):
            if tempBoard[r][c] == 1:
                ...
                for dir in ADJACENT_DIRECTIONS:
                    ...
                    if nr not in range(BOARD_SIZE) or nc not in range(BOARD_SIZE) or tempBoard[nr][nc] == 0:
                        emptySpace += 1
                if emptySpace > 3:
...
def evaluate_move(board, shape, row_idx, col_idx):
    ...
    for block in shape.borders:
        if row_idx+block.row not in range(BOARD_SIZE) or col_idx+block.col not in range(BOARD_SIZE):
            score +=1
        else:
            ...
            for turn in boardStates:
                ...
                for row_idx in range(BOARD_SIZE-shape.height+1):
                    for col_idx in range(BOARD_SIZE-shape.width+1):
                        ...
                        score, tempBoard = evaluate_move(turn.board, shape, row_idx, col_idx)
...
def generate_step_boards(board, shapes, winning_turn):
    ...
    for step, shape_idx in enumerate(winning_turn.order, 1):  # start=1 to use 1,2,3 as markers
        ...
        rows, columns = [], []
        for r in range(BOARD_SIZE):
            if sum(bool(current_board[r][c]) for c in range(BOARD_SIZE)) == BOARD_SIZE:  # using bool() to count any non-zero value
                rows.append(r)
                completed_rows += 1
        for c in range(BOARD_SIZE):
